...
function  [pi,x,i,ome_pi_tar,ome_pi_ext,ome_pi_ada,...
    ...
    = nkmbr_function_expect_final(del_pi,del_x,T,shock,eta,phi)
%% This function derives the time dependent path of inflation, output gap and the interest rate 
%  in a NKM with bounded rationality, where the expected values of
...
%  the expected values of the infaltion rate and the output gap.
...
%   average values
    pi_av = average(pi(1:t-1));
    x_av = average(x(1:t-1));
...
%   state matrices
    ...
    A = [(del_pi-1)*a_pi sig+(del_x-sig)*a_x;(1-bet*a_pi) -gam];
    
    B = [(del_pi-1)*b_pi (del_x-sig)*b_x;-bet*b_pi 0];
    
    C = [(1-del_pi)*c_pi (sig-del_x)*c_x;bet*c_pi 0];
        
    D = [(1-d_pi)*d_pi (sig-del_x)*d_x;bet*d_pi 0];
